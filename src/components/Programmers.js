...
export default function Programmers() {
  ...
  const isFeatured = featured.length > 0
  ...
  const getNameOfFeatured = () => {
    ...
    // let name;
    const foundPerson = programmers.find(programmer => programmer.id === featured)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: isFeatured ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
